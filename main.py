...
from telegram.ext import CallbackContext, MessageHandler, filters, ConversationHandler, CallbackQueryHandler, \
    ApplicationBuilder
import logging
...
async def play(update: Update, context: CallbackContext):
    ...
    if context.user_data['word'] >= len(context.user_data['sample']):
        return await stop(update, context, True)
    context.user_data['last_word'] = context.user_data['sample'][context.user_data['word']]
    await send_word(context.bot, update.effective_chat.id, context.user_data['last_word'])
    return PLAYING
...
async def stop(update: Update, context: CallbackContext, win=False):
